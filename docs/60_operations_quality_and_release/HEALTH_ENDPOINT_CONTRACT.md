...
- **CRE8-OPS-REQ-0025**: Liveness and readiness responses **MUST** use the canonical success envelope `{data, meta}` from [`API_CONTRACT_GUIDE.md`](API_CONTRACT_GUIDE.md) with `meta.request_id`, `meta.timestamp_utc`, and `meta.contract_version`.
...
- **CRE8-OPS-REQ-0044**: Readiness dependency checks **MUST** be reconciled with dependency inventory in [`docs/10_product_and_architecture/DEPENDENCY_BASELINE.md`](docs/10_product_and_architecture/DEPENDENCY_BASELINE.md) and required configuration variables in [`CONFIGURATION_ENVIRONMENT_CONTRACT.md`](CONFIGURATION_ENVIRONMENT_CONTRACT.md); unsupported or undocumented readiness dependencies **MUST NOT** be introduced.
