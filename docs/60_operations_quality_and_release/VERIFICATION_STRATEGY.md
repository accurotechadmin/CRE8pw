...
| HOOK-CONTRACT-ROUTE-INVENTORY-PARITY | PR | `composer docs:ssot:lint` and manual route diff (`ROUTE_INVENTORY_REFERENCE.md` vs `openapi/cre8.v1.yaml`) | Method/path and route_id parity maintained with no undocumented drift | reports/ssot/coverage_latest.json; docs/evidence/templates/README.md | Add `docs:ssot:route-parity` executable checker |
