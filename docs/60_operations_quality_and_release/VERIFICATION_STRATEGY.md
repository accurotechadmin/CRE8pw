...
| HOOK-CONTRACT-FEED-ORDER-CURSOR | PR | `composer test:contract:feed` | Feed fixtures include deterministic ordering and cursor semantics (`published_utc_desc__item_id_asc`; cursor points at final returned row) | reports/ssot/coverage_latest.json; docs/evidence/templates/README.md | Implemented as `test:contract:feed` |
