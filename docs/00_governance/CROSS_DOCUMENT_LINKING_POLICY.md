...
- **CRE8-GOV-REQ-0062**: A domain document under `docs/10_*` through `docs/80_*` **MUST** be reachable by at least one vertical path from [`README.md`](../../README.md) through [`SSOT_INDEX.md`](SSOT_INDEX.md).
...
- **CRE8-GOV-REQ-0065**: If a changed requirement ID is represented in [`TRACEABILITY_MATRIX.md`](../80_traceability_decisions_and_program/TRACEABILITY_MATRIX.md), the source document **MUST** preserve a stable heading anchor or provide a redirect note in the same pull request.

## Required link topology
1. **Vertical topology**: [`README.md`](../../README.md) → [`SSOT_INDEX.md`](SSOT_INDEX.md) → domain documents.
2. **Lateral topology**: domain documents ↔ dependent domain documents declared in `normative_dependencies`.
3. **Trace topology**: requirement-bearing docs → [`TRACEABILITY_MATRIX.md`](../80_traceability_decisions_and_program/TRACEABILITY_MATRIX.md) and related verification/evidence artifacts.
