...
## Purpose
Provide a complete implementation plan for three architecture upgrades using the current CRE8 PHP stack/dependencies:

1. **Alternative A** — Policy Decision Point (PDP) service-in-process.
2. **Alternative B** — BFF-by-surface (Gateway BFF + Console BFF in one Slim runtime).
3. **Alternative C** — Evented Command/Query split (CQRS-lite) with audit-first core.
...
Create an “architecture-upgrade checklist” requiring updates in same PR where behavior changes:
- OpenAPI
- route inventory
- API contract guide
- UI runtime contract
...
## 1) Alternative A implementation plan — PDP service-in-process
...
## 2) Alternative B implementation plan — BFF-by-surface
...
## 3) Alternative C implementation plan — CQRS-lite + audit-first
...
## 6) Definition of done per alternative
