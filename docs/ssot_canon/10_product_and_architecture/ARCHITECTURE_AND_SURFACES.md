...
- Gateway BFF read route families invoke `QueryBus` as the canonical read dispatch boundary; migrated read routes do not call read domain services directly and preserve canonical envelope and detail-code semantics.
- Console BFF read route families invoke `QueryBus` as the canonical read dispatch boundary; migrated read routes do not bypass query-handler orchestration and preserve canonical owner-governance response semantics.
- Gateway and console BFF write route families invoke `CommandBus` and `TransactionalCommandExecutor` as the canonical mutation dispatch boundary; migrated write routes do not call mutation domain services directly and preserve canonical `403/404/409/422` detail-code semantics.
