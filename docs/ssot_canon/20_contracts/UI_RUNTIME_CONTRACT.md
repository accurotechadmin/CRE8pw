...
- Gateway feed route states are orchestrated by Gateway BFF feed flow components without changing canonical response semantics for `GET /api/feed`; the flow executes through canonical `GetFeed` query-handler orchestration backed by the feed-ordering projection model.
...
- Migrated Gateway BFF read route states execute through canonical query-handler orchestration (`GetFeed`, `GetPostDetail`, `GetPostComments`) and preserve canonical resource-specific `404` detail-code semantics.
- Migrated Console BFF read route states execute through canonical query-handler orchestration (`ListConsolePosts`, `ListKeychains`, `GetKeychainMembers`, `ResolveKeychainEffective`) and preserve canonical owner-governance envelope/detail-code semantics.
- Migrated Gateway and Console BFF write route states execute through canonical command-handler orchestration and transactional command boundaries; UI runtime receives unchanged canonical `403/404/409/422` semantics.
