...
Mission
Understand the permissions/delegation stack using the repository SSOT (paths below).
Implement or author changes only within the user’s stated scope—small, reviewable batches.
Preserve strict SSOT discipline: stable requirement IDs (CRE8-*-REQ-*), hook IDs (HOOK-*), route IDs (CRE8-ROUTE-*), traceability, contract parity, verification hooks, and session continuity matching Phase 3/4 conventions (handoffs, progress board, archived verbatim response, verification before commit, change-impact maps when behavior/contracts shift).
Do not attempt full-platform completion in one run.
1) Mandatory orientation reads (before substantive edits)
Read at minimum the following. Log any missing paths explicitly in reports/session_handoffs/SESSION_HANDOFF_<UTC-YYYYMMDD-HHMM>.md.
...
docs/20_identity_delegation_and_policy/DRAFT_KEY_MINTING_PERMISSION_LATTICE.md — draft / non-normative; reconcile, do not silently canonize
...
docs/20_identity_delegation_and_policy/PERMISSION_VOCABULARY.md — read §Canonical permission registry, §Deprecated and legacy alias registry, and §Route inventory parity checklist
...
docs/10_product_and_architecture/ARCHITECTURE_AND_SURFACES.md — surface boundaries and route inventory ↔ OpenAPI parity obligations
...
docs/31_machine_contracts/README.md — how OpenAPI, schemas, and parity table fit together
docs/31_machine_contracts/openapi/cre8.v1.yaml — as needed for touched operations
docs/31_machine_contracts/PROSE_OPENAPI_PARITY_TABLE.md — when routes or error examples shift
...
docs/80_traceability_decisions_and_program/records/ADR-005-authz-gate-order-reconciliation.md — canonical PDP gate order decision
docs/80_traceability_decisions_and_program/SSOT_AUTOMATION_AND_LINTING.md — discover which composer docs:ssot:* scripts apply
...
dev/SSOT_CANON_READING_LIST.md — §4–§5 at minimum (identity + contracts); expand per slice
dev/SSOT_IMPLEMENTATION_MILESTONES_AND_SLICES.md — M7 (Identity / PDP / delegation) slices and verification hooks column
REFERENCE_MAINTENANCE_SOP.md — repository root (not under dev/); follow when adding/moving tracked files
Optional context (recent program artifacts — informational)

reports/phase4/P4-S2.2_PERMISSION_VOCAB_RECONCILIATION.md — example of vocabulary ↔ error catalog closure
...
Also follow dev/CRE8_PRODUCTION_CODEBASE_SESSION_PROMPT.md (canon/seed read-only for requirements; code changes isolated unless governance explicitly opens docs/).
...
docs/80_traceability_decisions_and_program/TRACEABILITY_MATRIX.md (and DECISIONS_LOG.md / RISK_REGISTER.md / ADRs if deferrals apply)
...
Then run area-specific checks from composer.json based on what you touched, for example:
...
Program bundles: run phase acceptance bundles when current CI/docs mandate them (see composer.json phase*: scripts and .github/workflows/).
...
Begin: complete orientation reads and state snapshot; ask the user what to do; remember to publish continuity artifacts and the structured final response at the end of the session.
