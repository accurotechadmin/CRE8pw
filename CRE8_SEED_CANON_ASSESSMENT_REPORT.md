...
| `CRE8_SEED_PRESERVATION_MATRIX.md` | Tracks preserved truths, redesign mandates, and dropped legacy assumptions. | ID-keypair-first is mandatory; handler-local auth dropped; auth-context interchangeability dropped; provenance bypass disallowed; follow-up specs required. | Preservation matrix, redesign concepts, dropped assumptions, carry-forward accountability. | Assumes next-phase docs are authored in full SSOT detail. | No closure ownership and due-order for listed follow-up items. |
