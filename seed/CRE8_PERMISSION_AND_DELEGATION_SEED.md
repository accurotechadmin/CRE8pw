...
## Dependency anchoring for delegation engine
Delegation policy modules SHOULD execute as PDP middleware chains in `slim/slim`, composed via `php-di/php-di`. Permission payloads SHOULD be validated using `respect/validation`; persistence and provenance mutation atomicity SHOULD use `ext-pdo` transactions; delegation decision traces SHOULD emit structured `monolog/monolog` records with correlation IDs.
